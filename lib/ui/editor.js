...
function dump() {
  ...
  output.log(
    'x: %d/%d, cur-x: %d/%d, y: %d/%d, cur-y',
    this.x, this.data[this.y].length-1, this.cursorY, profile.width,
    this.y, this.data.length-1, this.cursorY, profile.height)
}
...
Editor.prototype.backspace = function() {
  ...
  var overflow = Math.ceil(that.data.fake[refY].length / (profile.width-2)) - 1
  var totalOverflow = that.y - refY
  ...
  //
    ...
    // if (overflow && totalOverflow > 0) {
    //   for (var i = 0; i < totalOverflow; i++) {
    //     index += that.data[refY + i].length-2
    //   }
    // }
...
Editor.prototype.insert = function(ch, key) {
  ...
  if (chars.test(ch)) {
    ...
    if (that.cursorX < profile.width-2) {
      ...
      that.x = 0
      that.cursorX = 1
      that.program.cup(that.cursorY+1, profile.left)
    }
...
Editor.prototype.enter = function() {
  ...
  bounce = setTimeout(function() {
    ...
    var refY = that.data.rtof[that.y]
    output.log(refY, that.y, that.data.fake[refY])
    var line = that.data.fake[refY].split('')
...
Editor.prototype.left = function() {
  ...
  if (this.x > 0) {
    ...
    if (len > profile.width - 4) {
      len = profile.width - 4 // padding on both sides?
    }
...
Editor.prototype.right = function() {
  ...
  if (this.cursorX == profile.width-4 || // padding on both sides?
