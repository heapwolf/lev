...
Client.prototype.connect = function(info, cb) {
  ...
  if (!info) {
    that.logger.log('not enough connection information.')
    return
  }
...
Client.prototype.query = function(model, cb) {
  ...
  that.logger.log('createReadStream(%j)', model.query)

  model.query.values = false

  that.level.createReadStream(model.query)
  .on('data', function(k) {
    ++count
    keys.push(k)
  })
  .on('error', function(err) {
    that.logger.log('ERROR', model.query)
    cb(err)
  })
  .on('end', function() {
...
Client.prototype.getInfo = function(model, cb) {

  var that = this

  that.db.db.approximateSize(model.query.start, model.query.end, function(err, size) {
    if (err) return cb(err)
...
Client.prototype.selectSublevel = function(model, cb) {
  ...
  this.level = this.level.sublevel(model.key)
  this.query(model, cb)
}
