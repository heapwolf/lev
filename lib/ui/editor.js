...
function render() {
  ...
  output.log(
    ...
    this.x, this.data[this.y].length-1, this.cursorX, profile.width,
...
Editor.prototype.insert = function(ch, key) {
  ...
  if (chars.test(ch)) {
    ...
    if (that.cursorX <= profile.width) {
      ...
      var y = profile.top + that.cursorY
...
Editor.prototype.getProfile = function() {
  ...
  var border = this.parent.border ? 2 : 0
  var paddingV = 0 //(padding.top || 0) + (padding.bottom || 0)
  var paddingH = 0 //(padding.left || 0) + (padding.right || 0)
  var profile = {
    height: co.yl - co.yi - paddingV + border,
    width: co.xl - co.xi - paddingH + border,
    top: co.yi, // + padding.top,
    left: co.xi + (border ? 1 : 0) // + padding.left
  }
  return profile
}
