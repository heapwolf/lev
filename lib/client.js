...
Client.prototype.connect = function(opts, cb) {
  ...
  if (opts.local && !opts.a) {

    //
    // no reason to pass `reverse` to the ctor,
    // we woudn't be able to build a tree, copy it!
    //
    var levelopts = {}
    deepex(levelopts, opts)
    levelopts.reverse = false

    levelup(opts.path, levelopts, function(err, handle) {
  ...
  if (opts.a && typeof opts.a == 'string') {

    address = opts.a.split(':')
  ...
  else if (opts.a && typeof opts.a == 'number') {
    port = opts.a
  }

  if (opts.manifest) {
    manifest = require(path.resolve(opts.manifest))
  ...
  var conn = reconnect(function(stream) {
    ...
    rpc.on('error', function(err) { console.log(err) })
