...
function addTreeToCache(tree) {

  var p = subpath.join('')
  if (subpath.length == 1) {
    var list = Object.keys(tree).map(function(s) {
      return s + '/'
    })
    cachedkeys[p] = list.concat(cachedkeys[p])
  }
}
...
function setupContextMethods() {
  ...
  REPL.context.cd = function(opts) {
    ...
    else if (sl.length && sl.lastIndexOf('/') > 0) {
