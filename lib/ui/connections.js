...
function Connections(screen, model) {
  ...
  var textEncoding = blessed.textbox({
    parent: form,
    mouse: true,
    keys: true,
    bg: 243,
  ...
  textEncoding.on('focus', function() {
    textEncoding.readInput()
  })
  ...
  buttonSave.on('click', function() {
    ...
    var encoding = textEncoding.getValue()
    ...
    model.settings.connections[name].encoding = encoding
    model.settings.connections[name].local = checkType.checked
  ...
  buttonCreate.on('click', function() {
    ...
    model.settings.connections[name].encoding = 'json'

    textName.setValue(name)
    textPort.setValue(port)
    textHost.setValue(host)
    textManifest.setValue('')
    textEncoding.setValue('json')
