...
Editor.prototype.getContent = function() {
  ...
  var tmp = []
  return tmp.concat.apply(tmp, fake).join('\n')
}
...
Editor.prototype.refresh = function(autoscroll) {
  ...
  this.parent._wrapContent(content, co.xl - co.xi)
...
Editor.prototype.insert = function(ch, key) {
  ...
  if (key.ctrl && key.name == 's' && this.save) {
    return this.save(this.getContent())
  }
...
Editor.prototype.right = function() {
  ...
  if (this.cursorX == profile.width || 
    ...
    if (len > 0) {
      ...
      this.program.cud(profile.top + this.cursorY - 1)
